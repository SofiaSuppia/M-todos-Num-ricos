...
#define MAX_SIZE 100 
#include "gauss.h" // Se asume que este archivo existe e implementa gauss_elimination
 ...
 * @return 1 si es exitoso, 0 en caso contrario
 */
int leer_puntos_de_datos(const char* nombre_archivo, double X[], double Y[], int* n);
 ...
 */
void imprimir_puntos_de_datos(double X[], double Y[], int n);

/**
 * Función para imprimir los coeficientes de los splines cúbicos
 * @param X Arreglo de valores de X (puntos de datos)
 * @param solucion Arreglo que contiene los coeficientes de la solución
 * @param n Número de puntos de datos
 */
 ...
 * @param X Arreglo de valores de X (puntos de datos)
 * @param Y Arreglo de valores de Y (puntos de datos)
 * @param n Número de puntos de datos
 */
...
int main(int argc, char const *argv[]) {
    ...
    // Arreglos para el cálculo de coeficientes de polinomios (matriz A y vector b)
    double A[MAX_SIZE+1][MAX_SIZE+1], b[MAX_SIZE+1], solucion[MAX_SIZE+1];
    // n = número de puntos de datos, option = opción del menú
    int n, opcion, fila1, fila2;
    

    // Leer puntos de datos desde el archivo
    if (!leer_puntos_de_datos("data.txt", X, Y, &n)) {
        printf("Fallo al leer los datos del archivo. Saliendo.\n");
        return 1;
    }
    
    // Imprimir los puntos de datos
    imprimir_puntos_de_datos(X, Y, n);
    ...
    printf("Seleccione una opción para la interpolación por splines:\n");
    printf("1. Spline Lineal\n");
    printf("2. Spline Cúbico\n");
    scanf("%d", &opcion);
    ...
    switch(opcion) {
        case 1:
            ...
            while(opcion) {
                ...
                if(x_gorro < X[0] || x_gorro > X[n-1]) {
                    printf("x_gorro está fuera del rango [%lf, %lf]. Ingrese un valor dentro del rango.\n", X[0], X[n-1]);
                } else {
                    // Localizar el intervalo
                    int k = 0;
        ...
        break;
        case 2:
            // Inicializar A y b (ya hecho arriba, pero se repite en el original)
            for (int i = 0; i < 4*(n-1); i++) {
                b[i] = 0.0;
                for (int j = 0; j < 4*(n-1); j++) {
                    A[i][j] = 0.0;
            ...
            // Dos condiciones de frontera: Spline natural (segundas derivadas = 0 en los puntos extremos)
            fila1 = 4*(n-1) - 2;
            fila2 = 4*(n-1) - 1;
            ...
            // La función gauss_elimination debe recibir el tamaño de la matriz (4*(n-1))
            gauss_elimination(4*(n-1), A, b, solucion);
...
void imprimir_splines_cubicos(double X[], double solucion[], int n) {
    printf("------------------SOLUCIÓN------------------\n");
    printf("Coeficientes del Spline Cúbico:\n");
    for(int k = 0; k < n-1; k++) {
