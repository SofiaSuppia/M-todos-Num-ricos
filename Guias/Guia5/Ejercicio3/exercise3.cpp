...
#define MAX_PUNTOS 20 // Número máximo de puntos de datos
#define MAX_TAMANO 100 // Tamaño máximo para la matriz de coeficientes (para Vandermonde)
#include "gauss.h" // Incluye funciones para la Eliminación Gaussiana
 ...
 * @param Y Arreglo de valores Y
 * @param n Número de puntos de datos
 * @param coeficientes Arreglo para almacenar los coeficientes calculados
 */
void coeficientes_lagrange(double X[], double Y[], int n, double coeficientes[]);

/**
 * Función para imprimir la forma simbólica expandida del polinomio de Lagrange
 * @param coeficientes Arreglo de coeficientes
 * @param n Número de puntos de datos
 */
void imprimir_lagrange_expandido(double coeficientes[], int n);

/* Para analizar la aproximación de las mediciones y el polinomio de Lagrange realizamos los siguientes puntos:
--> Interpolamos en puntos intermedios y verificamos que sean realistas.
--> Extrapolamos a T = 500°C y analizamos si el resultado es físicamente razonable.
--> Graficamos el polinomio y verificamos que sigue el comportamiento esperado (cρ ∝ T³ para T→0, cρ→constante para T→∞).

Resultados:
--> Evaluar con x_gorro = 500°C es una extrapolación y obtuvimos el resultado de 8.772791 kJ/kg*K.
Este resultado es físicamente imposible.
--> Interpolando en puntos intermedios se obtienen valores realistas.
--> El gráfico está en esta carpeta, míralo para más información. Analizándolo podemos decir que el polinomio de Lagrange no es un buen ajuste
para este problema porque no sigue el comportamiento esperado de cρ→constante para T→∞.
Podríamos usar otros métodos como splines o ajuste polinomial por mínimos cuadrados para este tipo de problema.
 */

int main(int argc, char const *argv[]) {
    double X_gorro, suma, producto, error_calculado, f_real, Pn;
    ...
    if (!leer_puntos_datos("data.txt", X, Y, &n)) {
    ...
    printf("1. Interpolación de Lagrange (cálculo directo)\n");
    ...
    switch(opcion) {
        case 1:
            ...
            for(int k = 0; k < n; k++) {        
                ...
                // Podemos ver que la suma de Cnk = 1 (propiedad de los coeficientes)
            ...
            error_calculado = 0.0; // Si f_real es conocida, calcular error

            f_real = evaluar_funcion(X_gorro);
            error_calculado = calcular_error(f_real, suma);
        ...
        case 2:
            ...
            // Nota: Se asume que 'eliminacion_gaussiana' es el nombre de la función en 'gauss.h'
        ...
        case 3:
            ...
            double coeficientes[MAX_PUNTOS];
            coeficientes_lagrange(X, Y, n, coeficientes);
            
            printf("------------------POLINOMIO DE LAGRANGE------------------\n");
            imprimir_lagrange_expandido(coeficientes, n);

            // error = |f(T) - Pn(T)| 
            // Nota: Para datos experimentales no hay función teórica exacta
            f_real = evaluar_funcion(X_gorro);
            // El error_calculado usa la f_real (que en este caso es 0.0 por ser datos experimentales)
            error_calculado = calcular_error(f_real, Pn);

            // El mensaje de salida mantiene el contexto del problema original (calor específico vs. temperatura)
            printf("\nEl valor interpolado del calor específico en T = %.2f°C es: %.6f kJ/kg·K\n", X_gorro, Pn);
            if (f_real != 0.0) {
                printf("\nerror = |f(T) - Pn(T)| = %lf\n", error_calculado);
            } else {
                printf("\nNota: El error no se calcula - se usan datos experimentales sin función teórica exacta\n");
            }
            break;
...
double evaluar_funcion(double x) {
    // Para el problema del calor específico del aluminio vs. temperatura
    // No tenemos una función teórica exacta para comparar con los datos experimentales
    // El polinomio de Lagrange ES la aproximación que buscamos.
    
    // Nota: El mensaje de nota se imprime en main para no afectar otras opciones.
    return 0.0;  // Se devuelve 0.0 cuando no se conoce la función real
}
...
double polinomio_lagrange(double x, double X[], double Y[], int n) {
    ...
    for(int k = 0; k < n; k++) {        
        ...
        // Imprimir coeficiente Cnk(x) para referencia
...
void coeficientes_lagrange(double X[], double Y[], int n, double coeficientes[]) {
    ...
    for (int k = 0; k < n; k++) {
        double base[MAX_PUNTOS] = {1.0};  // Polinomio base, inicia en 1
        int grado = 0;
        ...
        for (int i = 0; i < n; i++) {
            ...
            // Multiplica por (x - X[i]) al polinomio base actual
        ...
        // Sumar la contribución del término k: Yk/Denominador * Polinomio_Base
        for (int j = 0; j <= grado; j++)
            coeficientes[j] += base[j] * factor;
    }
}
...
void imprimir_lagrange_expandido(double coeficientes[], int n) {
    ...
    int es_el_primero = 1;
    for (int i = 0; i < n; i++) {
        if (fabs(coeficientes[i]) < 1e-12) continue; // Ignorar coeficientes muy cercanos a cero
