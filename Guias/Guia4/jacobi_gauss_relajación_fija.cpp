...
/* Este archivo contiene correcciones de errores hechas con Github Copilot en los métodos de Gauss-Seidel y Relajación. */
 ...
 * Es similar a Gauss-Seidel pero con un factor de relajación omega y un paso adicional.
 * @param a Matriz de coeficientes
 * @param b Vector de términos independientes
 * @param Xv X Anterior = Solución de la iteración previa
 * @param Xn X Nuevo = Solución de la iteración actual
...
int main(int argc, char const *argv[]) {
    ...
    if (verificar_dominancia_diagonal(a, n) != 0) {
        printf("El método no puede continuar. La matriz tiene ceros en la diagonal. El programa finaliza.\n");
        return 1;
    }
...
void metodo_gauss_seidel(double a[][MAX_TAMANO+1], double b[], double Xv[], double Xn[], int n) {
    ...
    inicializar_estimacion(Xv, n);
    // Inicializar Xn con la estimación inicial también para el primer uso
    for(int i = 1; i <= n; i++) {
        Xn[i] = Xv[i];
    }
    ...
    do {
        ...
        for(int i = 1; i <= n; i++) {
            suma_temp = 0.0;  // Reiniciar suma para cada fila
            
            // Sumar elementos antes de la diagonal (usando los valores NUEVOS Xn)
            for(int j = 1; j <= i-1; j++) {
                suma_temp += a[i][j] * Xn[j];
            }
            
            // Sumar elementos después de la diagonal (usando los valores VIEJOS Xv)
            for(int j = i+1; j <= n; j++) {
...
void metodo_relajacion(double a[][MAX_TAMANO+1], double b[], double Xv[], double Xn[], int n) {
    ...
    do {
        ...
        for(int i = 1; i <= n; i++) {
            ...
            double gauss_seidel_temp = (b[i] - suma_temp) / a[i][i];
            
            // Aplicar el factor de relajación (SOR)
            // Xn[i] = omega * X_gauss_seidel + (1 - omega) * X_anterior
            Xn[i] = omega * gauss_seidel_temp + (1.0 - omega) * Xv[i];
        }
